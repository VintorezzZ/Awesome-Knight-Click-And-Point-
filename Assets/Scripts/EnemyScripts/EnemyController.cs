...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            enemy_CurrentState = SetEnemyState(enemy_CurrentState, enemy_LastState);
    ...
    private EnemyState SetEnemyState(EnemyState curState, EnemyState lastState)
    {
        float enemyToPlayerDist = Vector3.Distance(transform.position, playerTarget.position);
        ...
        else if (enemyToPlayerDist <= attack_Distance)
        {
            lastState = curState;
            curState = EnemyState.ATTACK;
        }
        else if (enemyToPlayerDist >= alert_Attack_Distance && lastState == EnemyState.PAUSE ||
                 lastState == EnemyState.ATTACK)
        {
            lastState = curState;
            curState = EnemyState.PAUSE;
        }
        else if (enemyToPlayerDist <= alert_Attack_Distance && enemyToPlayerDist > attack_Distance)
        {
        ...
        else if (enemyToPlayerDist > alert_Attack_Distance && lastState != EnemyState.GOBACK &&
    ...
    private void GetStateControl(EnemyState curState)
    {
        ...
        // if our agent controlls with char controller
